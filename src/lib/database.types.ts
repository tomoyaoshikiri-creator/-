...
// プラン体系(詳細はsrc/lib/plan.tsのPLAN_CONFIGを参照)。
// お試し/中間/フル/フルプラスはセルフ契約可能な通常販売プラン。
// Maxは正式販売プランだが個別見積もり制(セルフ契約不可、Stripe申し込み導線には出さない)。
// max_partnerはモニター・開発協力チーム向けの非売品、signature_editionは
// 都賀ビクトリーズ専用の完全非売品(いずれも運営が手動で付与する)。
export type TeamPlan = "お試し" | "中間" | "フル" | "フルプラス" | "Max" | "max_partner" | "signature_edition";
