...
export function TabBar({ role, badges = {} }: { role: Role; badges?: Partial<Record<TabKey, boolean>> }) {
  ...
  return (
    ...
    // 併せてアイコン/ラベルも少し拡大(icon17px→19px、label7px→8px)し、
    // 縦の余白とのバランスを取るためgapも0.5→1に調整した。
    // ナビ再設計v3でタブ数がロールに関わらず常に5個の固定になったため、8タブ時代の
    // dense(px-3)分岐は不要になった。5タブは旧7タブ時代よりさらに1タブあたりの幅に
    // 余裕があるため、旧non-dense(px-1)側の余白をそのまま使う。
    <nav className="min-[700px]:hidden flex items-start pt-2.5 pb-[env(safe-area-inset-bottom)] px-1 border-t border-line bg-white">
      {BOTTOM_NAV_TABS.map((tab) => {
        const Icon = TAB_ICONS[tab];
        const href = tabHrefForRole(role, tab);
        const isActive = pathname === href || pathname.startsWith(href + "/");
        const className = `relative flex-1 min-w-0 text-center text-[8px] font-medium flex flex-col items-center gap-1 ${
