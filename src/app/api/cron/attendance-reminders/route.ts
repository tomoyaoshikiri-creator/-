...
// あわせて、Vercel Cronのジョブ数を増やさないよう、選手の誕生日プッシュ通知(runBirthdayReminders)も
// この日次バッチの中でまとめて処理する。退会猶予期間を過ぎたチームの完全削除は、
// push通知(VAPIDキー)の設定有無に処理が左右されないよう、専用cron
// (/api/cron/team-deletion)に分離している(A-6)。
