...
export default function CoachNoteDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (r) {
      ...
      const [{ data: atts }, { data: rc }, { data: cm }] = await Promise.all([
        ...
        supabase.from("report_comments").select("*").eq("report_id", r.id).order("created_at", { ascending: true }),
      ]);
  ...
  async function loadReactions() {
    if (!report) return;
    const supabase = createClient();
    const { data } = await supabase.from("report_reactions").select("*").eq("report_id", report.id);
    setReactions(data ?? []);
  }
  ...
  async function toggleReaction(type: ReactionType) {
    ...
    if (existing) {
      ...
      // リアクションはコーチ日報本体を更新しないため、そのままだと一覧・タブのNEW表示
  ...
  async function toggleCommentReaction(commentId: string, type: ReactionType) {
    ...
    if (existing) {
      ...
      // リアクションはコメント自体・コーチ日報本体のどちらも更新しないため、そのままだと
      // コメント個別のNEW表示・一覧/タブのNEW表示のどちらにも反映されない。両方の
      // updated_atを更新して拾われるようにする(失敗してもリアクション自体は成功して
      // いるため、ベストエフォートでエラー表示はしない)。
      const now = new Date().toISOString();
      await Promise.all([
        supabase.from("report_comments").update({ updated_at: now }).eq("id", commentId),
        report ? supabase.from("reports").update({ updated_at: now }).eq("id", report.id) : Promise.resolve(),
      ]);
    }
  ...
  return (
    <PageShell
      header={
        <AppHeader
          title={report ? formatFullDateLabel(report.date) : "コーチ日報"}
          variant="detail"
          backHref="/coach-note"
          accessBadge="coach"
        />
    ...
    >
      {loading ? (
        ...
        <EmptyState>コーチ日報が見つかりません</EmptyState>
      ) : editing ? (
        <>
          <SectionLabel>コーチ日報を編集</SectionLabel>
          <Card>
          ...
          <Card>
            ...
            >
              {deleting ? "削除中…" : deleteConfirm ? "もう一度タップで削除確定" : "このコーチ日報を削除する"}
            </button>
          </Card>
        </>
      ) : (
