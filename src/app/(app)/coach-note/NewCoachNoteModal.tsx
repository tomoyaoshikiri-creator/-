...
export function NewCoachNoteModal({
  ...
  async function handleSubmit() {
    ...
    for (const file of files) {
      ...
      if (uploadError) {
        const cleanupOk = await cleanupUploadedObjects(
          supabase,
          uploaded.map((u) => ({ bucket: "report-attachments", path: u.path })),
        );
        setSaving(false);
        toast(
    ...
    for (const u of uploaded) {
      ...
      if (attachError) {
        const rollbackOk = await rollbackParentAndObjects(supabase, {
          parentTable: "reports",
          parentId: report.id,
          uploadedObjects: uploaded.map((x) => ({ bucket: "report-attachments", path: x.path })),
        });
        setSaving(false);
        toast(
    ...
    setSaving(false);
    reset();
    sendPushNotification("coach_note_created", report.id);
    onCreated();
  }

  return (
    <Modal open={open} onClose={onClose} title="コーチ日報を書く">
