...
export default function PlayerAnalysisPage() {
  ...
  const { role, userId, teamId, plan, sport, category } = useSession();
  const toast = useToast();
  const isStaff = canViewKarte(role);
  const backHref = `/karte/players/${params.playerId}`;

  useEffect(() => {
    if (!isStaff || !hasKarteTabAccess(plan) || !hasAiAnalysisAccess(plan)) router.replace(backHref);
  }, [isStaff, plan, router, backHref]);

  const [analysisOpen, setAnalysisOpen] = useState(false);
  const [extractFiscalYear, setExtractFiscalYear] = useState(CURRENT_FISCAL_YEAR);
  ...
  useEffect(() => {
    markItemSeenAndGetPrevious(userId, "player_analysis", params.playerId).then(setPrevSeenAt);
  ...
  async function handleCopyAnalysis() {
    ...
    try {
      const supabase = createClient();
      const data = await collectPlayerAnalysisData(supabase, {
        playerId: params.playerId,
        fiscalYear: extractFiscalYear,
        sport,
        category,
        planKind,
      });
  ...
  return (
    <PageShell header={<AppHeader title="選手分析" variant="detail" backHref={backHref} accessBadge="coach" />}>
      ...
      {notesLoading ? (
        ...
        <CollapsibleList
          ...
          renderItem={(n) => (
            <Link key={n.id} href={`/karte/players/${params.playerId}/notes/${n.id}`}>
      ...
      {role === "管理者" && hasAiAnalysisAccess(plan) && (
        <Modal open={analysisOpen} onClose={() => setAnalysisOpen(false)} title="分析用抽出">
          ...
          <InlineSelect
            className="mb-3"
            value={String(extractFiscalYear)}
            onChange={(v) => setExtractFiscalYear(Number(v))}
