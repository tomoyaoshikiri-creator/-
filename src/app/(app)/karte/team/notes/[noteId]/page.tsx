...
export default function TeamAnalysisNoteDetailPage() {
  const params = useParams<{ noteId: string }>();
  const router = useRouter();
  const { role, userId } = useSession();
  const toast = useToast();

  const [note, setNote] = useState<TeamAnalysisNote | null>(null);
  const [reactions, setReactions] = useState<TeamAnalysisNoteReaction[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: n }, profMap] = await Promise.all([
      supabase.from("team_analysis_notes").select("*").eq("id", params.noteId).maybeSingle(),
      loadProfilesMap(supabase),
    ]);
    setNote(n ?? null);
    setProfiles(profMap);
    if (n) {
      const { data: r } = await supabase.from("team_analysis_note_reactions").select("*").eq("note_id", n.id);
      setReactions(r ?? []);
    } else {
      setReactions([]);
    }
  ...
  async function handleDelete() {
    ...
    const supabase = createClient();
    const { error } = await supabase.from("team_analysis_notes").delete().eq("id", note.id);
    if (error) {
