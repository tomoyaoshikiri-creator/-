...
例外的に、指導者・管理者しか遷移できないページ(選手一覧・OB/OG・試合記録の記録画面)には`AppHeader`の`accessBadge="coach"`で「コーチ専用画面」、管理者しか遷移できないページ(ユーザー管理・配色・ロゴ)には`accessBadge="admin"`で「管理者専用画面」というバッジをタイトル横に表示し、そのページが誰向けかを画面上でも分かるようにしている。選手詳細ページ(`/players/[id]`)は保護者が自分の子どもの分だけ閲覧できる例外があるためバッジは付けていない。
...
src/
  app/
    ...
    (app)/schedule, schedule/[id], notice,        予定(一覧は要約カード→詳細で出欠登録)・お知らせ(全ロール)
          report, players, players/[id],         日報・選手一覧(選手詳細でメモの閲覧・登録も行う)・
...
- 招待リンクを取り消す(無効化する)UIが無い。発行したリンクは有効期限が切れるまで誰でも使える(`invites`にDELETEポリシーも無い)
