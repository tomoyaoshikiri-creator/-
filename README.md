...
- 試合(`game_matches`)には対戦相手に加えて自チーム/相手チームの得点とスコア写真を記録できる。勝敗は得点から自動判定して表示するため、専用の列は持たない
- スコア写真は1試合につき1枚のみで、Storageバケット`game-score-photos`(非公開、指導者・管理者のみアクセス可)に保存する
