...
例外的に、指導者・管理者しか遷移できないページ(選手一覧・OB/OG・試合記録の記録画面)には`AppHeader`の`accessBadge="coach"`で「コーチ専用画面」、管理者しか遷移できないページ(配色・ロゴ)には`accessBadge="admin"`で「管理者専用画面」というバッジをタイトル横に表示し、そのページが誰向けかを画面上でも分かるようにしている。選手詳細ページ(`/players/[id]`)は保護者が自分の子どもの分だけ閲覧できる例外があるためバッジは付けていない。「ユーザー管理」は役員も一部(招待リンク発行)にアクセスできるため、専用バッジは付けていない。
